...
namespace ConnectWars
{
    ...
    C_SpeedUpOption::C_SpeedUpOption(const std::string& rId, int32_t type) : C_BaseOption(rId, type)
    {
        ...
        // モデル情報を取得
        assert(OpenGL::C_PrimitiveBufferManager::s_GetInstance()->GetPrimitiveBuffer(ID::Primitive::s_pSPEED_UP_OPTION));
        pModelData_ = OpenGL::C_PrimitiveBufferManager::s_GetInstance()->GetPrimitiveBuffer(ID::Primitive::s_pSPEED_UP_OPTION).get();

        // テクスチャ情報を取得
        assert(Texture::C_TextureManager::s_GetInstance()->GetTextureData(Path::Texture::s_pSPPED_UP_OPTION));
        pModelTextureData_ = Texture::C_TextureManager::s_GetInstance()->GetTextureData(Path::Texture::s_pSPPED_UP_OPTION).get();
