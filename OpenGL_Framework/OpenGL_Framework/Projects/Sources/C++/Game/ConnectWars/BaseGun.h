...
namespace ConnectWars
{
    ...
    public:
        ...
        void SetShotOffset(const Vector3& rShotOffset, bool defaultFlag = true);            // 射撃のオフセットを設定
        void SetBulletId(const std::string& rBulletId);                                     // 弾のIDを設定
        void SetBulletMoveLogic(C_MoveLogic* pBulletMoveLogic);                             // 弾の移動ロジックを設定
        void SetBulletPower(C_BasePower* pBulletPower);                                     // 弾の攻撃力を設定
        void SetAutoBulletFowardDirectionFlag(bool autoBulletFowardDirectionFlag);          // 弾の前向きを自動化するフラグを設定
    protected:
        ...
        std::shared_ptr<C_MoveLogic> upBulletMoveLogic_;                                    ///< @brief 弾の移動ロジック
    };
}
