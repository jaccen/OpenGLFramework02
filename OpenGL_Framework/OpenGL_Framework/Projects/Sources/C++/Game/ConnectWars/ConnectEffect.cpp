...
namespace ConnectWars
{
     ...
     *
     ****************************************************************/
    C_ConnectEffect::C_ConnectEffect(const std::string& rId, int32_t type) : C_BaseEffect(rId, type)
    {
     ...
     *
     ****************************************************************/
    C_ConnectEffect::~C_ConnectEffect()
    {
    }
    ...
    void C_ConnectEffect::DoUpdate()
    {
        if (wpCircleParticleSystem_.expired() == false && wpRingParticleSystem_.expired() == false)
        {
            ...
            catch (luabind::error &rErrorInfo)
            {
                ...
                PrintLog("[ C_ConnectEffect::DoUpdate ] : 関数の呼び出しに失敗しました。\n");
                PrintLog("                エラーメッセージ : %s\n", stream.str().c_str());
     ...
     *
     ****************************************************************/
    bool C_ConnectEffect::DoMessageProcess(const Telegram& rTelegram)
    {
        return true;
    }
}