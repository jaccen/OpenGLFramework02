...
namespace ConnectWars
{
     ...
     *
     ****************************************************************/
    C_PlayerFlarebackEffect::C_PlayerFlarebackEffect(const std::string& rId, int32_t type) : C_BaseEffect(rId, type)
    {
     ...
     *
     ****************************************************************/
    C_PlayerFlarebackEffect::~C_PlayerFlarebackEffect()
    {
    }
     ...
     *
     ****************************************************************/
    void C_PlayerFlarebackEffect::DoUpdate()
    {
        if (wpCircleParticleSystem_.expired() == false && wpSmokeParticleSystem_.expired() == false)
        {
            ...
            catch (luabind::error &rErrorInfo)
            {
                ...
                PrintLog("[ C_PlayerFlarebackEffect::DoUpdate ] : 関数の呼び出しに失敗しました。\n");
                PrintLog("                エラーメッセージ : %s\n", stream.str().c_str());
     ...
     *
     ****************************************************************/
    void C_PlayerFlarebackEffect::DoDraw()
    {
    }
     ...
     *
     ****************************************************************/
    bool C_PlayerFlarebackEffect::DoMessageProcess(const Telegram& rTelegram)
    {
        return true;
    }
}