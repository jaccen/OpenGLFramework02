...
namespace ConnectWars
{
     ...
     *
     ****************************************************************/
    C_OptionExplosionEffect::C_OptionExplosionEffect(const std::string& rId, int32_t type) : C_BaseEffect(rId, type)
    {
     ...
     *
     ****************************************************************/
    C_OptionExplosionEffect::~C_OptionExplosionEffect()
    {
    }
     ...
     *
     ****************************************************************/
    void C_OptionExplosionEffect::DoUpdate()
    {
        if (wpCircleParticleSystem_.expired() == false && wpSmokeParticleSystem_.expired() == false)
        {
            ...
            catch (luabind::error &rErrorInfo)
            {
                ...
                PrintLog("[ C_OptionExplosionEffect::DoUpdate ] : 関数の呼び出しに失敗しました。\n");
                PrintLog("                エラーメッセージ : %s\n", stream.str().c_str());
     ...
     *
     ****************************************************************/
    void C_OptionExplosionEffect::DoDraw()
    {
    }
     ...
     *
     ****************************************************************/
    bool C_OptionExplosionEffect::DoMessageProcess(const Telegram& rTelegram)
    {
        return true;
    }
}