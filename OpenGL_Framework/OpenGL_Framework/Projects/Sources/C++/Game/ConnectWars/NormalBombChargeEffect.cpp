...
namespace ConnectWars
{
     ...
     *
     ****************************************************************/
    C_NormalBombChargeEffect::C_NormalBombChargeEffect(const std::string& rId, int32_t type) : C_BaseEffect(rId, type)
    {
     ...
     *
     ****************************************************************/
    C_NormalBombChargeEffect::~C_NormalBombChargeEffect()
    {
    }
     ...
     *
     ****************************************************************/
    void C_NormalBombChargeEffect::DoUpdate()
    {
        if (wpCircleParticleSystem_.expired() == false && wpRingParticleSystem_.expired() == false)
        {
            ...
            catch (luabind::error &rErrorInfo)
            {
                ...
                PrintLog("[ C_NormalBombChargeEffect::DoUpdate ] : 関数の呼び出しに失敗しました。\n");
                PrintLog("                エラーメッセージ : %s\n", stream.str().c_str());
     ...
     *
     ****************************************************************/
    void C_NormalBombChargeEffect::DoDraw()
    {
    }
     ...
     *
     ****************************************************************/
    bool C_NormalBombChargeEffect::DoMessageProcess(const Telegram& rTelegram)
    {
        return true;
    }
}