...
namespace ConnectWars
{
     ...
     *
     ****************************************************************/
    C_PlayerBeamCollisionEffect::C_PlayerBeamCollisionEffect(const std::string& rId, int32_t type) : C_BaseEffect(rId, type)
    {
     ...
     *
     ****************************************************************/
    C_PlayerBeamCollisionEffect::~C_PlayerBeamCollisionEffect()
    {
    }
     ...
     *
     ****************************************************************/
    void C_PlayerBeamCollisionEffect::DoUpdate()
    {
        if (wpCircleParticleSystem_.expired() == false && wpRingParticleSystem_.expired() == false)
        {
            ...
            try
            {
                ...
                existenceFlag_ = luabind::call_function<bool>(pLuaState_.get(),
                                                              "PlayerBeamCollisionEffectLogic",
            ...
            catch (luabind::error &rErrorInfo)
            {
                ...
                PrintLog("[ C_PlayerBeamCollisionEffect::DoUpdate ] : 関数の呼び出しに失敗しました。\n");
                PrintLog("                エラーメッセージ : %s\n", stream.str().c_str());
     ...
     *
     ****************************************************************/
    void C_PlayerBeamCollisionEffect::DoDraw()
    {
    }
     ...
     *
     ****************************************************************/
    bool C_PlayerBeamCollisionEffect::DoMessageProcess(const Telegram& rTelegram)
    {
        return true;
    }
}